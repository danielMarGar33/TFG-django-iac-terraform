...
# Creacion de la red interna
resource "openstack_networking_network_v2" "Daniel_control_network" {
  name           = "Daniel_control_network"
  admin_state_up = true
}
...
resource "openstack_networking_subnet_v2" "Daniel_control_subnetwork" {
  ...
  network_id = openstack_networking_network_v2.Daniel_control_network.id
  cidr       = "10.1.0.0/16"
  gateway_ip = "10.1.0.1"
}
...
# subred_UE_AGF = 10.0.0.0/24
# subred_AGF_core5G = 10.0.1.0/24
# subred_core5G_server = 10.0.2.0/24
...
resource "openstack_networking_subnet_v2" "Daniel_core5G_server_subnetwork" {
  name       = "Daniel_core5G_server_subnetwork"
  network_id = openstack_networking_network_v2.Daniel_network.id
  cidr       = "10.0.2.0/24"
}
...
resource "openstack_networking_port_v2" "Daniel_core5G_server_port" {
  name       = "Daniel_core5G_server_port"
  network_id = openstack_networking_network_v2.Daniel_network.id
  fixed_ip {
    subnet_id = openstack_networking_subnet_v2.Daniel_core5G_server_subnetwork.id
  }
  timeouts {
    create = "10m"
    delete = "10m"
...
resource "openstack_compute_instance_v2" "Daniel_core5G" {
  ...
  flavor_id = "101"
  network {
    port = openstack_networking_port_v2.Daniel_core5G_server_port.id
  }
  network {
...
resource "openstack_networking_port_v2" "Daniel_server_control_port" {
  name       = "Daniel_server_control_port"
  network_id = openstack_networking_network_v2.Daniel_control_network.id
  fixed_ip {
    subnet_id = openstack_networking_subnet_v2.Daniel_control_subnetwork.id
  }
...
resource "openstack_networking_port_v2" "Daniel_server_core5G_port" {
  name       = "Daniel_server_core5G_port"
  network_id = openstack_networking_network_v2.Daniel_network.id
  fixed_ip {
    subnet_id = openstack_networking_subnet_v2.Daniel_core5G_server_subnetwork.id
  }
...
resource "openstack_compute_instance_v2" "Daniel_server" {
  name      = "Daniel_server"
  image_id  = "db02fc5c-cacc-42be-8e5f-90f2db65cf7c"
  flavor_id = "101"
  network {
    port = openstack_networking_port_v2.Daniel_server_core5G_port.id
  }
  network {
    port = openstack_networking_port_v2.Daniel_server_control_port.id
  }
}
